...
class QuestionController extends Controller
{
    ...
    public function update(Request $request, Question $question)
    {
        ...
        $validated = $request->validate([
            ...
            'difficulty_level' => ['sometimes', 'required', 'string', \Illuminate\Validation\Rule::in(array_column(DifficultyLevelEnum::cases(), 'value'))],
            'timer' => ['sometimes', 'required', 'integer', \Illuminate\Validation\Rule::in(array_column(TimerEnum::cases(), 'value'))],
            'score_value' => ['sometimes', 'required', 'integer', \Illuminate\Validation\Rule::in(array_column(QuestionScoreEnum::cases(), 'value'))],
        ...
        DB::transaction(function () use ($request, $question, $validated) {
            // 2. Update Question - Only update provided fields
            $updateData = [];
            ...
            if (isset($validated['difficulty_level'])) {
                $updateData['difficulty_level'] = $validated['difficulty_level'];
            }
            if (isset($validated['timer'])) {
                $updateData['timer'] = $validated['timer'];
            }
            if (isset($validated['score_value'])) {
                $updateData['score_value'] = $validated['score_value'];
            }
            ...
            // 3. Handle Options (only if provided)
