 ...
 * @see app/Http/Controllers/Admin/QuestionBankController.php:121
...
generateWithAI.url = (args: { questionBank: string | { id: string } } | [questionBank: string | { id: string } ] | string | { id: string }, options?: RouteQueryOptions) => {
    ...
    return generateWithAI.definition.url
            .replace('{questionBank}', parsedArgs.questionBank.toString())
            .replace(/\/+$/, '') + queryParams(options)
}
...
const QuestionBankController = { downloadTemplate, index, create, store, show, edit, update, destroy, uploadQuestions, generateWithAI, generateTags }
